...
public:
	...
	public:
		Position() : CRect(0,0,0,0), m_szMin(0) { memset(m_Bytes, 0, 4); }
...
private:
	// DON'T USE
	cdxCDynamicWnd(const cdxCDynamicWnd & w);
	void operator=(const cdxCDynamicWnd & w);
	// helpers
...
inline bool cdxCDynamicWnd::AddSzControl(HWND hwnd, const SBYTES & bytes, const CSize & szMin, bool bReposNow)
{
	if(!::IsWindow(hwnd))
	{
		...
		TRACE(_T("*** NOTE[cdxCDynamicWnd::AddSzControl(HWND,const SBYTES &,const CSize &,bool)]: Handle 0x%p is not a valid window.\n"), hwnd);
		return false;
	}
...
inline bool cdxCDynamicWnd::AddSzControl(HWND hwnd, HWND hLikeThis, bool bReposNow)
{
	if(!::IsWindow(hwnd))
	{
		TRACE(_T("*** NOTE[cdxCDynamicWnd::AddSzControl(HWND,HWND,bool)]: Handle 0x%p is not a valid window.\n"), hwnd);
		return false;
	}
	...
	{
		TRACE(_T("*** NOTE[cdxCDynamicWnd::AddSzControl(HWND,HWND,bool)]: For the 'hLikeThis' handle 0x%p there hasn't been made an entry for yet.\n"), hLikeThis);
		return false;
	}
