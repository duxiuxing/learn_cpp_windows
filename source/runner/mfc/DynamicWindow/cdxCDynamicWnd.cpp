...
bool cdxCDynamicWnd::AddSzControl(HWND hwnd, const Position & pos, bool bReposNow)
{
	...
	if(!::IsWindow(hwnd))
	{
		TRACE(_T("*** NOTE[cdxCDynamicWnd::AddSzControl(HWND,const Position &,bool)]: Handle 0x%p is not a valid window.\n"), hwnd);
		return false;
	}
...
bool cdxCDynamicWnd::UpdateControlPosition(HWND hwnd)
{
	...
	if(!::IsWindow(hwnd))
	{
		TRACE(_T("*** NOTE[cdxCDynamicWnd::UpdateControlPosition()]: Handle 0x%p is not a valid window.\n"), hwnd);
		return false;
	}
...
void cdxCDynamicWnd::Layout()
{
	...
	try
	{
		...
		{
			TRACE(_T("*** NOTE[cdxCDynamicWnd::Layout()]: The window at 0x%p does not have child windows.\n"), m_pWnd->m_hWnd);
			return;
		}
